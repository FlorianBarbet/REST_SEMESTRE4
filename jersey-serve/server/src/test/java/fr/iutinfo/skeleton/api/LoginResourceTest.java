...
public class LoginResourceTest extends JerseyTest {
    ...
    public void should_return_current_user_with_authorization_header() {
        ...
        UserJobber utilisateur = target(url).request().header(AUTHORIZATION, authorization).get(UserJobber.class);
        assertEquals("tclavier", utilisateur.getName());
    }

    @Test
    public void should_return_anonymous_user_without_authorization_header() {
        UserJobber utilisateur = target(url).request().get(UserJobber.class);
